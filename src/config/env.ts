// eslint-disable-next-line @typescript-eslint/no-explicit-any -- window.__BEKRAFTABESLUT_FE_ENV__ is injected at runtime via runtime-config.js and has no type definition
function readRuntimeGlobal(): any {
  return (window as any).__BEKRAFTABESLUT_FE_ENV__ ?? {};
}
...
export const env = {
  ...
  devHandlaggningId: readRuntimeGlobal().RUNTIME_DEV_HANDLAGGNING_ID || import.meta.env.VITE_DEV_HANDLAGGNING_ID || "",
};
...
// index.html, so that <script> tag never runs and window.__BEKRAFTABESLUT_FE_ENV__
